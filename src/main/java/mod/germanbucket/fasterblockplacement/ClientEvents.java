        ...
        public static void onClientTick(TickEvent.ClientTickEvent event) {
            ...
            if(TOGGLE_INSTAPLACE_BIND.get().isDown() && Minecraft.getInstance().player != null && ticker <= 0) {
                ...
                if(Config.isInstantPlacing) {
                    Minecraft.getInstance().player.displayClientMessage(Component.translatable("message." + FasterBlockPlacement.MODID + ".disabledinstaplace"), true);
                    Minecraft.getInstance().player.playSound(SoundEvents.NOTE_BLOCK_PLING.get());
                    Config.isInstantPlacing = false;
                } else {
                    Minecraft.getInstance().player.displayClientMessage(Component.translatable("message." + FasterBlockPlacement.MODID + ".enabledinstaplace"), true);
